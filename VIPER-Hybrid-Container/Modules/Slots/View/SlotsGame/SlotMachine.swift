...
class SlotMachine {
    ...
    init(scene:SKScene, frame:CGRect, numberOfColumns:Int, columnSpacing:CGFloat, numberOfRows:Int, slotsStartAtIndex:Int = 0, spinDirection:Slot.SpinDirection = .downwards ) {
        self.frame = frame ;
...
//                print("Found! \(columnIndex) \(rowIndex) \(found)")
    ...
    func addFoundRectangle(_ columnIndex: Int , _ rowIndex:Int, _ found: Int) {
        ...
        let shapeNode = SKShapeNode(rect: CGRect(x:frame.origin.x + CGFloat(startColumn) * (slotWidth+columnSpacing), y:frame.origin.y - CGFloat(numberOfRows - rowIndex) * slotHeight, width: slotWidth * CGFloat(found + 1) + columnSpacing * CGFloat(found), height: slotHeight))
